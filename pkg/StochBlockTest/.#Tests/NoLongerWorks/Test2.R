...
stochBlock(M,clu,weights = W,  diagonal =  "seperate", limits =lim)

stochBlockORP(M,k=5,rep=100,weights = W,  diagonal =  "seperate")
system.time(res<-stochBlockORP(M[,,1],k=5,rep=100,  diagonal =  "seperate"))

system.time(res<-stochBlockORP(M[,,1],k=5,rep=100))
...
critFunction( M, clu3, W, sum(nClu),n, weightClusterSize = 1, "seperate", "inside", bordersLower, bordersUpper, bordersSeperateLower, bordersSeperateUpper )
stochBlock(M,clu3,W,sum(n),sum(nClu), "seperate", "inside", bordersLower, bordersUpper, bordersSeperateLower, bordersSeperateUpper )
...
clu3<-sample(c(0:4, 0:4))
clu3
stochBlock(M,clu3,weights = W,  diagonal =  "seperate", limits =lim2)
stochBlock(M,clu3,weights = W,  diagonal =  "seperate", limits =lim)
stochBlock(M,clu3,weights = W,  diagonal =  "seperate")
...
for(i in 1:1000){
  clu3<-sample(c(0:4, 0:4))
  tmp1<-stochBlock(M,clu3,weights = W,  diagonal =  "seperate", limits =lim3)
  tmp2<-stochBlock(M,clu3,weights = W,  diagonal =  "seperate", limits =lim)
  tmp3<-stochBlock(M,clu3,weights = W,  diagonal =  "seperate")
...
StochBlockTest::kmBlock(M[,,1,drop=FALSE],clu4,W,n,nClu, "ignore", "inside", bordersLower, bordersUpper )
...
StochBlockTest::kmBlock(M[,,1,drop=FALSE],clu4,W,n,nClu, "seperate", "inside", bordersLower, bordersUpper, bordersSeperateLower, bordersSeperateUpper )
#blockmodelingTest::kmBlock(M[,,1],clu = clu4)[c("clu","err")]
...
(tmp<-StochBlockTest::kmBlock(M[,,1,drop=FALSE],clu5,W,n,nClu))
