...
#' @param justMove Should just moves (so and not also exchanges) be allowed. The default value is set to \code{FALSE} mainly for backcompatibility reasons. If \code{TRUE}, the search is not so thorough, but is much faster. 
#' @param nImp Only used if \code{useMulti} is \code{FALSE}. In this case, it determines how many better partitions than the original one have to be found before we move to a better partition. Defaults to 1 for backcompatibility reasons and as this is experimental feature. 
#' @param maxPar The number of partitions with optimal criterion function to be returned. Only used If \code{useMulti} is \code{TRUE}.
...
optParC<-function(M, clu, approaches, blocks, nMode=NULL,isSym=NULL,diag=1, useMulti=FALSE, justMove = FALSE, nImp = 1, maxPar=50, IM=NULL,EM=NULL,Earr=NULL, justChange=TRUE, sameIM=FALSE, regFun="max", homFun = "ss", usePreSpecM = NULL, preSpecM=NULL, minUnitsRowCluster = 1, minUnitsColCluster = 1, maxUnitsRowCluster = 9999, maxUnitsColCluster = 9999, relWeights=1, posWeights=1, blockTypeWeights=1,combWeights=NULL, exchageClusters="all",fixClusters = NULL, save.initial.param=TRUE, mulReg=TRUE, addGroupLlErr=TRUE){
